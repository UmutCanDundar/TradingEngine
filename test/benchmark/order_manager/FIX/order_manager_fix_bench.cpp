...
public:
    ...
    void SetUp(const ::benchmark::State& st) 
    {
        ...
        ord_manager_traffic = []()
        {
            ...
            arr[1] = MessageWithVenue<MessageTypes_t>{&test_data_ordMngr::fix_new, Venue::BIST};  

            // 3 — FIX Partial Fill (GARAN, 200 filled)
            arr[2] = MessageWithVenue<MessageTypes_t>{&test_data_ordMngr::fix_partial, Venue::BIST};

            // 4 — FIX Cancel Confirm (GARAN)
            arr[3] = MessageWithVenue<MessageTypes_t>{&test_data_ordMngr::fix_cancel, Venue::BIST};
...
BENCHMARK_DEFINE_F(BM_OrderManager, Fix)(benchmark::State& state)
{
    pin_to_cpu(6);
...
BENCHMARK_REGISTER_F(BM_OrderManager, Fix)->UseManualTime();
