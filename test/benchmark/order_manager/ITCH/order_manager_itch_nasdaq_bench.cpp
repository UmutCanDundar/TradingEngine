...
public:
    ...
    void SetUp(const ::benchmark::State& st) 
    {
        ...
        ord_manager_traffic = []()
        {
            ...
            // 3 — NASDAQ ITCH OrderExecuted (AAPL, 60 executed)
            arr[2] = MessageWithVenue<MessageTypes_t>{NASDAQ::ITCHMessage{&test_data_ordMngr::nasdaq_exec}, Venue::NASDAQ}; 

            // 4 — NASDAQ ITCH OrderDelete (AAPL, remaining 40 deleted)
            arr[3] = MessageWithVenue<MessageTypes_t>{NASDAQ::ITCHMessage{&test_data_ordMngr::nasdaq_delete}, Venue::NASDAQ};
...
BENCHMARK_DEFINE_F(BM_OrderManager, ItchNasdaq)(benchmark::State& state)
{
    pin_to_cpu(6);
...
BENCHMARK_REGISTER_F(BM_OrderManager, ItchNasdaq)->UseManualTime();
