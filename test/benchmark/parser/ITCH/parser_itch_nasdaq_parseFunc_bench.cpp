...
static void BM_Parser_ITCH_NASDAQ_parseFunc(benchmark::State& state)
{
    pin_to_cpu(0);
    ...
    const uint8_t* data = [pkt_idx]()
    {
        switch(pkt_idx)
        {
            case 1: return test_data_parser::itch_nasdaq_pkt1.data(); 
            case 2: return test_data_parser::itch_nasdaq_pkt2.data(); 
            case 3: return test_data_parser::itch_nasdaq_pkt3.data(); 
...
BENCHMARK(BM_Parser_ITCH_NASDAQ_parseFunc)->Arg(1)->ArgName("StockDir");  
BENCHMARK(BM_Parser_ITCH_NASDAQ_parseFunc)->Arg(2)->ArgName("AddOrder");
BENCHMARK(BM_Parser_ITCH_NASDAQ_parseFunc)->Arg(3)->ArgName("Exec");    
