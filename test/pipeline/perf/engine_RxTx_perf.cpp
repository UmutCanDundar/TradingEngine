...
int main()
{
    ...
    auto run_case = [&](int pkt_case, int ord_case)
    {
        uint64_t before_rx = engine->risk_.pipeline_seq.load(std::memory_order_acquire);
        uint64_t before_tx = engine->network_io_.pipeline_seq.load(std::memory_order_acquire);
        ...
        while (engine->risk_.pipeline_seq.load(std::memory_order_acquire) <= before_rx)
            _mm_pause();
