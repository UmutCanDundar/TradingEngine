...
RESULT="$RESULT_DIR/engine_Tx_results.txt"
...
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Tx_cpu_pcore.data" \
...
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Tx_cpu_ecore.data" \
    --call-graph graph,0.5,caller,function,percent 2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Tx_dcache.data" 2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Tx_icache.data" 2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Tx_tlb.data" 2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
perf c2c record -g --call-graph dwarf \
    -o "$PERF_DATA_DIR/engine_Tx_c2c.data" \
    -- "$TEST_BIN" >/dev/null 2>&1 &
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf c2c report --stdio --stats \
    -i "$PERF_DATA_DIR/engine_Tx_c2c.data" 2>/dev/null || true
...
if command -v stackcollapse-perf.pl &>/dev/null && \
   command -v flamegraph.pl &>/dev/null; then
    ...
    PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
    SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
    perf script -i "$PERF_DATA_DIR/engine_Tx_flame.data" \
        | stackcollapse-perf.pl \
        | flamegraph.pl > "$RESULT_DIR/flamegraph_engine_Tx.svg"
    echo "Flamegraph saved: $RESULT_DIR/flamegraph_engine_Tx.svg"
else
