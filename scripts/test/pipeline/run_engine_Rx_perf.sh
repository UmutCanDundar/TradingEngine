...
TEST_BIN="$ROOT/build/test/pipeline/perf/engine_Rx_perf"
RESULT_DIR="$ROOT/test/_results/perf"
RESULT="$RESULT_DIR/engine_Rx_results.txt"
...
perf record -F 99999 -g -m 256M \
    -e cpu_core/cycles/,cpu_core/instructions/,cpu_core/branches/,cpu_core/branch-misses/ \
    -o "$PERF_DATA_DIR/engine_Rx_cpu_pcore.data" \
    -- "$TEST_BIN" >/dev/null 2>&1 &
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Rx_cpu_pcore.data" \
...
perf record -F 99999 -g -m 256M \
    -e cpu_atom/cycles/,cpu_atom/instructions/,cpu_atom/branches/,cpu_atom/branch-misses/ \
    -o "$PERF_DATA_DIR/engine_Rx_cpu_ecore.data" \
    -- "$TEST_BIN" >/dev/null 2>&1 &
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Rx_cpu_ecore.data" \
    --call-graph graph,0.5,caller,function,percent 2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
perf record -F 99999 -g -m 256M \
    -e cpu_core/L1-dcache-load-misses/,cpu_core/LLC-load-misses/ \
    -o "$PERF_DATA_DIR/engine_Rx_dcache.data" \
    -- "$TEST_BIN" >/dev/null 2>&1 &
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Rx_dcache.data" 2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
perf record -F 99999 -g -m 256M \
    -e cpu_core/L1-icache-load-misses/ \
    -e cpu_atom/L1-icache-load-misses/ \
    -o "$PERF_DATA_DIR/engine_Rx_icache.data" \
    -- "$TEST_BIN" >/dev/null 2>&1 &
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Rx_icache.data" 2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
perf record -F 99999 -g -m 256M \
    -e cpu_core/dTLB-load-misses/,cpu_core/iTLB-load-misses/ \
    -e cpu_atom/dTLB-load-misses/,cpu_atom/iTLB-load-misses/ \
    -o "$PERF_DATA_DIR/engine_Rx_tlb.data" \
    -- "$TEST_BIN" >/dev/null 2>&1 &
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf report --stdio -i "$PERF_DATA_DIR/engine_Rx_tlb.data" 2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
perf c2c record -g --call-graph dwarf \
    -o "$PERF_DATA_DIR/engine_Rx_c2c.data" \
    -- "$TEST_BIN" >/dev/null 2>&1 &
PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
perf c2c report --stdio --stats \
    -i "$PERF_DATA_DIR/engine_Rx_c2c.data" 2>/dev/null || true
...
if command -v stackcollapse-perf.pl &>/dev/null && command -v flamegraph.pl &>/dev/null; then
    perf record -F 99999 -g -m 64M -F 9999 \
        ...
        -o "$PERF_DATA_DIR/engine_Rx_flame.data" \
        -- "$TEST_BIN" >/dev/null 2>&1 &
    PERF_PID=$!; sleep 60 && kill -SIGINT $PERF_PID 2>/dev/null &
    SLEEP_PID=$!; wait $PERF_PID 2>/dev/null || true; kill $SLEEP_PID 2>/dev/null || true
    perf script -i "$PERF_DATA_DIR/engine_Rx_flame.data" \
        | stackcollapse-perf.pl | flamegraph.pl \
        > "$RESULT_DIR/flamegraph_engine_Rx.svg"
    echo "Flamegraph saved: $RESULT_DIR/flamegraph_engine_Rx.svg"
else
