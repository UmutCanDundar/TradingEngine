...
RESULT_UPDATE="$RESULT_DIR/risk_update_results.txt"
RESULT_CHECK="$RESULT_DIR/risk_check_results.txt"
...
# ==================================================
(
    exec > >(tee -a "$RESULT_UPDATE") 2>&1

    system_info

    run_section "1. PERF STAT - BASE"
    perf stat -d "$TEST_BIN1" 2>&1 || true
    ...
    perf report --stdio \
        -i "$PERF_DATA_DIR/risk_update_cpu.data" \
    ...
    perf record -g -m 256M \
        ...
        "$TEST_BIN1" >/dev/null 2>&1 || true
    perf report --stdio \
        -i "$PERF_DATA_DIR/risk_update_dcache.data" \
        2>/dev/null \
       | grep -E "^\s+[0-9]+\.[0-9]+%" \
       | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
       | head -20 || true
    ...
    perf record -g -m 256M \
        ...
        "$TEST_BIN1" >/dev/null 2>&1 || true
    perf report --stdio \
        -i "$PERF_DATA_DIR/risk_update_icache.data" \
        2>/dev/null \
       | grep -E "^\s+[0-9]+\.[0-9]+%" \
       | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
       | head -20 || true
    ...
    perf record -g -m 256M \
        ...
        "$TEST_BIN1" >/dev/null 2>&1 || true
    perf report --stdio \
        -i "$PERF_DATA_DIR/risk_update_tlb.data" \
        2>/dev/null \
       | grep -E "^\s+[0-9]+\.[0-9]+%" \
       | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
       | head -20 || true
    ...
    perf c2c record -g --call-graph dwarf \
        -o "$PERF_DATA_DIR/risk_update_c2c.data" \
        -- "$TEST_BIN1" >/dev/null 2>&1 || true
    perf c2c report --stdio --stats \
        -i "$PERF_DATA_DIR/risk_update_c2c.data" \
        2>/dev/null || true
    ...
    if command -v stackcollapse-perf.pl &>/dev/null && \
       command -v flamegraph.pl &>/dev/null; then
        ...
        -e cpu_core/cycles/ \
            ...
            -- "$TEST_BIN1" >/dev/null 2>&1 || true
        perf script -i "$PERF_DATA_DIR/risk_update_flame.data" \
            | stackcollapse-perf.pl \
            | flamegraph.pl > "$RESULT_DIR/flamegraph_risk_update.svg"
        echo "Flamegraph saved: $RESULT_DIR/flamegraph_risk_update.svg"
    else
        echo "FlameGraph tools not found — skipping."
    fi

    echo ""
    echo "================ END RUN (RISK UPDATE) ================"
    echo ""
    echo "Results: $RESULT_UPDATE"
)
...
# ==================================================
(
    exec > >(tee -a "$RESULT_CHECK") 2>&1

    system_info

    run_section "1. PERF STAT - BASE"
    perf stat -d "$TEST_BIN2" 2>&1 || true
    ...
    perf report --stdio \
        -i "$PERF_DATA_DIR/risk_check_cpu.data" \
        --call-graph graph,0.5,caller,function,percent \
        2>/dev/null \
       | grep -E "^\s+[0-9]+\.[0-9]+%" \
       | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    ...
    perf record -g -m 256M \
        ...
        "$TEST_BIN2" >/dev/null 2>&1 || true
    perf report --stdio \
        -i "$PERF_DATA_DIR/risk_check_dcache.data" \
        2>/dev/null \
       | grep -E "^\s+[0-9]+\.[0-9]+%" \
       | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
       | head -20 || true
    ...
    perf record -g -m 256M \
        ...
        "$TEST_BIN2" >/dev/null 2>&1 || true
    perf report --stdio \
        -i "$PERF_DATA_DIR/risk_check_icache.data" \
        2>/dev/null \
       | grep -E "^\s+[0-9]+\.[0-9]+%" \
       | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
       | head -20 || true
    ...
    perf record -g -m 256M \
        ...
        "$TEST_BIN2" >/dev/null 2>&1 || true
    perf report --stdio \
        -i "$PERF_DATA_DIR/risk_check_tlb.data" \
        2>/dev/null \
       | grep -E "^\s+[0-9]+\.[0-9]+%" \
       | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
       | head -20 || true
    ...
    run_section "10. C2C - FALSE SHARING SUMMARY"
    perf c2c record -g --call-graph dwarf \
        -o "$PERF_DATA_DIR/risk_check_c2c.data" \
        -- "$TEST_BIN2" >/dev/null 2>&1 || true
    perf c2c report --stdio --stats \
        -i "$PERF_DATA_DIR/risk_check_c2c.data" \
        2>/dev/null || true
    ...
    if command -v stackcollapse-perf.pl &>/dev/null && \
       command -v flamegraph.pl &>/dev/null; then
        ...
        -e cpu_core/cycles/ \
            ...
            -- "$TEST_BIN2" >/dev/null 2>&1 || true
        perf script -i "$PERF_DATA_DIR/risk_check_flame.data" \
            | stackcollapse-perf.pl \
            | flamegraph.pl > "$RESULT_DIR/flamegraph_risk_check.svg"
        echo "Flamegraph saved: $RESULT_DIR/flamegraph_risk_check.svg"
    else
        echo "FlameGraph tools not found — skipping."
    fi

    echo ""
    echo "================ END RUN (RISK CHECK) ================"
    echo ""
    echo "Perf data files: $PERF_DATA_DIR"
    echo "Results: $RESULT_CHECK"
)