...
RESULT="$RESULT_DIR/builder_results.txt"
...
BLUE='\033[0;34m'
NC='\033[0m'
echo -e "\n\n${BLUE}Running builder perf test...${NC}"
...
perf report --stdio \
    -i "$PERF_DATA_DIR/builder_cpu.data" \
...
perf record -F 99999 -g -m 256M \
    ...
    "$TEST_BIN" >/dev/null 2>&1 || true
perf report --stdio \
    -i "$PERF_DATA_DIR/builder_dcache.data" \
    2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
perf record -F 99999 -g -m 256M \
    ...
    "$TEST_BIN" >/dev/null 2>&1 || true
perf report --stdio \
    -i "$PERF_DATA_DIR/builder_icache.data" \
    2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
perf record -F 99999 -g -m 256M \
    ...
    "$TEST_BIN" >/dev/null 2>&1 || true
perf report --stdio \
    -i "$PERF_DATA_DIR/builder_tlb.data" \
    2>/dev/null \
    | grep -E "^\s+[0-9]+\.[0-9]+%" \
    | awk '{match($0,/[0-9]+\.[0-9]+/); if (substr($0,RSTART,RLENGTH)+0 >= 1.0) print}' \
    | head -20 || true
...
# ==================================================
# C2C - FALSE SHARING (SUMMARY ONLY)
# ==================================================

run_section "12. C2C - FALSE SHARING SUMMARY"
perf c2c record -g --call-graph dwarf \
    -o "$PERF_DATA_DIR/builder_c2c.data" \
    -- "$TEST_BIN" >/dev/null 2>&1 || true
perf c2c report --stdio --stats \
    -i "$PERF_DATA_DIR/builder_c2c.data" \
    2>/dev/null || true

# ==================================================
# FLAMEGRAPH (SVG - separate file)
# ==================================================
...
if command -v stackcollapse-perf.pl &>/dev/null && \
   command -v flamegraph.pl &>/dev/null; then
    perf record -F 99999 -g -m 256M  \
        ...
        -- "$TEST_BIN" >/dev/null 2>&1 || true
    perf script -i "$PERF_DATA_DIR/builder_flame.data" \
        | stackcollapse-perf.pl \
        | flamegraph.pl > "$RESULT_DIR/flamegraph_builder.svg"
    echo "Flamegraph saved: $RESULT_DIR/flamegraph_builder.svg"
else
