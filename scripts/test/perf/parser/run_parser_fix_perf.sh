...
TEST_BIN="$ROOT/build/test/perf/parser/FIX/parser_fix_perf"
RESULT_DIR="$ROOT/test/_results/perf/parser"
RESULT="$RESULT_DIR/parser_fix_results.txt"
...
if command -v stackcollapse-perf.pl &>/dev/null && command -v flamegraph.pl &>/dev/null; then
    ...
    perf script -i "$PERF_DATA_DIR/fix_flame.data" \
        ...
        > "$RESULT_DIR/flamegraph_parser_fix.svg"
    echo "Flamegraph saved: $RESULT_DIR/flamegraph_parser_fix.svg"
else
