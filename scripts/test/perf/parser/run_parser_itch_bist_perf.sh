#!/usr/bin/env bash
# run_parser_itch_bist_perf.sh
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
TEST_BIN="$ROOT/build/test/perf/parser/ITCH/parser_itch_bist_perf"
RESULT_DIR="$ROOT/test/_results/perf/parser"
RESULT="$RESULT_DIR/parser_itch_bist_results.txt"
...
if command -v stackcollapse-perf.pl &>/dev/null && command -v flamegraph.pl &>/dev/null; then
    ...
    perf script -i "$PERF_DATA_DIR/itch_bist_flame.data" \
        ...
        > "$RESULT_DIR/flamegraph_parser_itch_bist.svg"
    echo "Flamegraph saved: $RESULT_DIR/flamegraph_parser_itch_bist.svg"
else
