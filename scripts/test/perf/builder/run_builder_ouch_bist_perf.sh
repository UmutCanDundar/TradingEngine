...
TEST_BIN="$ROOT/build/test/perf/builder/OUCH/builder_ouch_bist_perf"
RESULT_DIR="$ROOT/test/_results/perf/builder"
RESULT="$RESULT_DIR/builder_ouch_bist_results.txt"
...
if command -v stackcollapse-perf.pl &>/dev/null && command -v flamegraph.pl &>/dev/null; then
    ...
    perf script -i "$PERF_DATA_DIR/ouch_bist_flame.data" \
        ...
        > "$RESULT_DIR/flamegraph_builder_ouch_bist.svg"
    echo "Flamegraph saved: $RESULT_DIR/flamegraph_builder_ouch_bist.svg"
else
