...
TEST_BIN="$ROOT/build/test/perf/builder/FIX/builder_fix_perf"
RESULT_DIR="$ROOT/test/_results/perf/builder"
RESULT="$RESULT_DIR/builder_fix_results.txt"
...
if command -v stackcollapse-perf.pl &>/dev/null && command -v flamegraph.pl &>/dev/null; then
    ...
    perf script -i "$PERF_DATA_DIR/fix_flame.data" \
        ...
        > "$RESULT_DIR/flamegraph_builder_fix.svg"
    echo "Flamegraph saved: $RESULT_DIR/flamegraph_builder_fix.svg"
else
